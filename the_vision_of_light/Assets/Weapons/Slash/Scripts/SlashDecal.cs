...
namespace MaykerStudio.VFX
{
    ...
    public class SlashDecal : MonoBehaviour
    {
        ...
        bool IsGrounded(out Vector3 collisionPoint)
        {
            ...
            if (Physics.CapsuleCast(point1, point2, capsuleRadius, -up, out RaycastHit hit, maxDistance, groundLayer))
            {
                collisionPoint = hit.point;
                return true;
            }
            else if (Physics.CapsuleCast(point1, point2, capsuleRadius, up, out hit, maxDistance, groundLayer))
            {
                collisionPoint = hit.point;
                return true;
            }
