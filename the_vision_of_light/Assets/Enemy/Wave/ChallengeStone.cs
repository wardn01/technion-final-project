...
public class ChallengeStone : MonoBehaviour
{
    public enum ChallengeStoneType
    {
        ...
        OneTime,
        /// <summary>Respawns on a cooldown for training; quest progress only on first clear.</summary>
        RepeatableTraining
    }
    ...
    [Tooltip("Seconds until the interact prompt returns after a repeatable run.")]
    public float trainingRespawnSeconds = 300f;
    ...
    [Header("Quest (RepeatableTraining — first clear only)")]
    public QuestChallenge[] questChallenges;

    [Header("Waves")]
    [Tooltip("One-time stones require this. Repeatable stones use it for training after quest is cleared.")]
    ...
    private bool CanShowPrompt()
    {
        ...
        return ChallengeTrialRegistry.IsRepeatableReady(trialId);
    }
    #endregion
    ...
    private Wave[] ResolveActiveWaves()
    {
        ...
        Wave[] training = ResolveTrialWaves();
        if (training != null && training.Length > 0)
            return training;

        return GetFallbackQuestWaves();
    }
    ...
    private Wave[] GetFallbackQuestWaves()
    {
        if (questChallenges == null || questChallenges.Length == 0)
            return null;

        Wave[] fallback = questChallenges[0].waves;
    ...
    private bool TryResolveQuestChallenge(out QuestChallenge match)
    {
        ...
        if (stoneType != ChallengeStoneType.RepeatableTraining)
            return false;

        if (questChallenges == null || questChallenges.Length == 0)
            return false;

        if (ChallengeTrialRegistry.IsQuestClearClaimed(trialId))
            return false;
    ...
    private void CompleteChallengeSuccess()
    {
        ...
        if (activeQuestChallenge != null && QuestManager.Instance != null)
        {
            ...
            ChallengeTrialRegistry.MarkQuestClearClaimed(trialId);
        }
        ...
        {
            ChallengeTrialRegistry.StartRepeatableCooldown(trialId, trainingRespawnSeconds);
        }
