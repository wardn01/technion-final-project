...
public class CharacterMenuController : MonoBehaviour
{
    ...
    public Vector2 menuOpenPosition =
        new Vector2(0, -420);
    ...
    public Color selectedBuildColor =
        Color.white;

    public Color unselectedBuildColor =
    ...
    private void Start()
    {
        ...
        if (quickSlotsBar)
            normalPosition =
                quickSlotsBar.anchoredPosition;

        if (openAttributesBtn)
        {
            openAttributesBtn.onClick.AddListener(() =>
            {
                SwitchTab(true);
            });
        }
        ...
        if (openWeaponsBtn)
        {
            openWeaponsBtn.onClick.AddListener(() =>
            {
                SwitchTab(false);
            });
        }
        ...
        if (buildTabBtns != null)
        {
            for (int i = 0; i < buildTabBtns.Length; i++)
            {
                ...
                buildTabBtns[i].onClick.AddListener(() =>
                {
                    SelectBuildTab(index);
                });
            }
        }
    ...
    public void ToggleMenu()
    {
        ...
        if (isOpening)
        {
            ...
            {
                mainContentPanel.anchoredPosition =
                    Vector2.zero;
            }
        }
        else
        {
            ...
            if (contentSlideCoroutine != null)
            {
                StopCoroutine(contentSlideCoroutine);
            }
        }
    }
    ...
    public void SwitchTab(bool showAttributes)
    {
        ...
        {
            attributesBtnText.color =
                showAttributes
                ? activeTabColor
                : inactiveTabColor;

            attributesBtnText.text =
                showAttributes
                ? "♦ Attributes"
                : "  Attributes";

            attributesBtnText.transform.localScale =
                showAttributes
        ...
        if (weaponsBtnText)
        {
            ...
            weaponsBtnText.transform.localScale =
                !showAttributes
                ? new Vector3(1.1f, 1.1f, 1f)
                : Vector3.one;
        }
        ...
        {
            attributesSubPanel
                .GetComponent<PlayerAttributesUI>()
                ?.RefreshUI();
        }
        else
        {
            weaponsSubPanel
                .GetComponent<WeaponUpgradeUI>()
                ?.RefreshGrid();
        }
    }
    ...
    private void SelectBuildTab(int index)
    {
        ...
        if (
            index == currentSelectedBuildSlot &&
            !isInitializing
        )
        {
            return;
        ...
        else
        {
            ...
            contentSlideCoroutine =
                StartCoroutine(
                    SlideContentAndLoad(index)
                );
        }
    }

    private IEnumerator SlideContentAndLoad(
        int newIndex
    )
    {
        if (mainContentPanel == null)
        ...
        float direction =
            (newIndex > currentSelectedBuildSlot)
            ? -1f
            : 1f;

        Vector2 startPos = Vector2.zero;

        Vector2 targetOutPos =
            new Vector2(
    ...
    private void UpdateBuildData(int index)
    {
        ...
        if (!isInitializing)
        {
            playerData.SaveBuild(
                currentSelectedBuildSlot
            );
        }

        currentSelectedBuildSlot = index;

        playerData.LoadBuild(
            currentSelectedBuildSlot
        );
            ...
            for (int i = 0; i < buildTabBtns.Length; i++)
            {
                ...
                if (btnImg != null)
                {
                    btnImg.color =
                        (i == index)
                        ? selectedBuildColor
                        : unselectedBuildColor;
                }

                if (
                    buildTabTexts != null &&
                    buildTabTexts.Length > i &&
                    buildTabTexts[i] != null
                )
                {
                    buildTabTexts[i].color =
                        (i == index)
                        ? Color.black
                        : Color.white;
                }
