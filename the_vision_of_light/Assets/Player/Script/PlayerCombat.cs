...
public class PlayerCombat : MonoBehaviour
{
    ...
    )
    {
        ...
        {
            anim.runtimeAnimatorController =
                activeWeaponData.animatorOverride;
        }
        else
        {
            anim.runtimeAnimatorController =
        ...
        if (
            activeWeaponData.weaponModelPrefab != null &&
            weaponHandPosition != null
        )
        {
            currentWeaponModel =
                Instantiate(
                    activeWeaponData.weaponModelPrefab,
                    weaponHandPosition
                );

            currentWeaponModel.SetActive(
                inCombatStance
            );
        }
    ...
    public bool IsSafeToEquip()
    {
        ...
        if (
            combatState.IsName("Empty") ||
            combatState.IsName("Combat_Movement")
        )
        {
            return true;
        }
    ...
    public bool CanInterrupt()
    {
        ...
        if (
            state.IsName("Skill_E") ||
            state.IsName("Skill_Q")
        )
        {
            return state.normalizedTime >= 0.95f;
        }
        else if (
            state.IsName("Rolling") ||
            state.IsName("Attack_1") ||
    ...
    private void HandleInput()
    {
        ...
        if (!CanInterrupt() || activeWeaponData == null)
            return;
        ...
        if (
            ...
            activeWeaponData.requiredE_For_Q
        )
        {
            ForceCancelRoll();
        ...
        )
        {
            ...
            state.skillETimer =
                activeWeaponData.skillECooldown;
    ...
    private void CheckAttackState()
    {
        ...
        if (
            stateInfo.IsName("Empty") ||
            stateInfo.IsName("Combat_Movement") ||
            stateInfo.IsName("Rolling")
        )
        {
            isAttacking = false;
        }
    ...
    private void OnAnimatorMove()
    {
        ...
        bool isAlwaysMovingSkill =
            stateInfo.IsName("Skill_Q") ||
            stateInfo.IsName("Rolling");
    ...
    private int GetWeaponUpgradeBoost(
        WeaponItemData weapon
    )
    {
        if (playerData == null || weapon == null)
            return 0;

        int currentLvl =
            playerData.GetWeaponLevel(
                weapon.itemName
            );
    ...
    public void DealNormalDamage()
    {
        ...
        float playerBaseAttack =
            playerData != null
            ? playerData.GetTotalAttack()
            : 0f;

        int weaponBoost =
            GetWeaponUpgradeBoost(
                activeWeaponData
            );

        float totalAttack =
            playerBaseAttack +
            activeWeaponData.weaponBaseAttack +
            weaponBoost;

        float finalDamage =
            totalAttack *
            (
                activeWeaponData.normalAttackDamage
                / 100f
            );
    ...
    public void OnSkillE()
    {
        if (
            activeWeaponData != null &&
            activeWeaponData.skillEPrefab != null &&
            eSpawnPoint != null
        )
        {
            GameObject skill =
                Instantiate(
                    activeWeaponData.skillEPrefab,
                    eSpawnPoint.position,
                    eSpawnPoint.rotation
                );
            ...
            float finalDamage =
                totalAttack *
                (
                    activeWeaponData.skillEDamage
                    / 100f
                );

            WindSkillEDamage windScript =
                skill.GetComponent<WindSkillEDamage>();
            ...
            IceSkillEDamage iceScript =
                skill.GetComponent<IceSkillEDamage>();
    ...
    public void OnSkillQ()
    {
        if (
            activeWeaponData != null &&
            activeWeaponData.skillQPrefab != null &&
            qSpawnPoint != null
        )
        {
            GameObject skill =
            ...
            IceSkillQDamage iceScript =
                skill.GetComponent<IceSkillQDamage>();

            if (iceScript != null)
            {
                skill.transform.parent =
                    qSpawnPoint;

                iceScript.SetDamage(finalDamage);
            }

            WindSkillQDamage windScript =
                skill.GetComponent<WindSkillQDamage>();
    ...
    public void PlayNormalAttackSound()
    {
        ...
        {
            currentWeaponModel
                .GetComponent<WeaponCombatSounds>()
